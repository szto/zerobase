...
├── assets/css/input.css # Tailwind 진입점 (@source 로 템플릿 스캔) — 빌드 소스, 서빙 안 됨
├── static/              # 서빙되는 정적 파일 (output.css/alpine.min.js 는 빌드 산출물)
...
# 3) 실행 (터미널 2개)
./tailwindcss -i assets/css/input.css -o static/css/output.css --watch
...
- LiveView 에는 `ZDebugViewMixin` 을 섞는다 → 프로덕션에서도 URL 에 `?zdebug` 를
  붙이면 djust 디버그 패널이 뜬다 (외부 공개 서비스는 `ZDEBUG_ENABLED=false` 로 차단)
...
- 새 Django 앱 추가 시 `assets/css/input.css` 에 `@source` 경로 추가
- Tailwind 소스(`assets/`)와 서빙 정적 파일(`static/`)을 섞지 않는다 —
  WhiteNoise manifest 가 소스 파일의 `@import` 를 참조로 해석해 collectstatic 이 깨진다
